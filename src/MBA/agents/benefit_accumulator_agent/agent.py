"""
Benefit Accumulator Agent initialization and configuration.

This module establishes the Strands-orchestrated agent that integrates
AWS Bedrock language models with RDS MySQL benefit accumulator data for
benefit usage inquiry workflows.

The agent is initialized with:
- Bedrock runtime client (claude-sonnet-4.5 via AWS SDK)
- Benefit accumulator lookup tools
- Strict JSON response system prompt
- Production-grade logging and error handling

Architecture:
    User Request → Strands Agent → Bedrock LLM → get_benefit_accumulator Tool → RDS MySQL
                                                                                   ↓
...
try:
    ...
    logger.info(
        f"Bedrock configuration ready for Benefit Accumulator Agent",
        extra={
    ...
    logger.error(f"Failed to set up AWS credentials for Benefit Accumulator Agent: {str(e)}")
    raise ConfigError(
...
benefit_accumulator_agent = Agent(
    name="BenefitAccumulatorAgent",
    system_prompt=BENEFIT_ACCUMULATOR_SYSTEM_PROMPT,
    tools=[get_benefit_accumulator],
    model=settings.bedrock_model_id  # Pass model ID string directly
)

logger.info(
    "Benefit Accumulator Agent initialized successfully",
    extra={
        "agent_name": benefit_accumulator_agent.name,
        "tools_count": 1,  # get_benefit_accumulator tool
