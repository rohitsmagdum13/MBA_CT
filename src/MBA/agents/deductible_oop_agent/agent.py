"""
Deductible/OOP Agent initialization and configuration.

This module establishes the Strands-orchestrated agent that integrates
AWS Bedrock language models with RDS MySQL deductible/OOP data for
benefit inquiry workflows.

The agent is initialized with:
- Bedrock runtime client (claude-sonnet-4.5 via AWS SDK)
- Deductible/OOP lookup tools
- Strict JSON response system prompt
- Production-grade logging and error handling

Architecture:
    User Request → Strands Agent → Bedrock LLM → get_deductible_oop Tool → RDS MySQL
                                                                              ↓
...
try:
    ...
    logger.info(
        f"Bedrock configuration ready for Deductible/OOP Agent",
        extra={
    ...
    logger.error(f"Failed to set up AWS credentials for Deductible/OOP Agent: {str(e)}")
    raise ConfigError(
...
deductible_oop_agent = Agent(
    ...
    system_prompt=DEDUCTIBLE_OOP_SYSTEM_PROMPT,
    tools=[get_deductible_oop],
    model=settings.bedrock_model_id  # Pass model ID string directly
)

logger.info(
    "Deductible/OOP Agent initialized successfully",
    extra={
        "agent_name": deductible_oop_agent.name,
        "tools_count": 1,  # get_deductible_oop tool
